...
- **PatientData**: The binary matrix containing mutation information.
- **PatientSensitivity**: The matrix that provides drug response information.
...
- **tree**: The trained decision tree obtained from the `trainTree` function.
- **PatientData**: The binary matrix containing mutation information.
- **PatientSensitivity**: The matrix containing drug response information.
...
<div style="text-align: center;">
  <img src="MutationalExample.png" alt="Trained Decision Tree Output" width="50%" />
...
- **PatientData**: The matrix containing gene expression information.
- **PatientSensitivity**: The matrix providing drug response information.
- **minbucket**: A fixed parameter that specifies the minimum number of patients required in a branch of the tree to allow a split.
...
# Load mutation and drug response data
data("mutations_w34")
data("mutations_w12")
...
In this example, we will use a matrix containing gene expression information along with a drug response matrix from existing patients. We will train a model using the *ODT* package to predict the best treatment for a new patient whose sensitivity response to different treatments is unknown.
